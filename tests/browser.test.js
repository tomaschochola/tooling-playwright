...
test('launches every configured browser project with its declared context', async (testContext) => {
    ...
    for (const project of [...createPlaywrightProjects(), ...createPlaywrightPhoneProjects(), ...createPlaywrightTabletProjects()]) {
        await testContext.test(project.name, async () => {
            ...
            if (browser === undefined) {
                ...
                testContext.after(async () => await browser.close(), { timeout: 10000 });
            }
...
test('settles real browser resources and accepts an accessible document', async (testContext) => {
...
test('settles relevant resources without triggering deferred lazy images in every browser engine', async (testContext) => {
    ...
    for (const [browserName, browserType] of Object.entries(browserTypes)) {
        await testContext.test(browserName, async (engineTestContext) => {
            const browser = await browserType.launch();

            engineTestContext.after(async () => await browser.close(), { timeout: 10000 });
...
test('reports an Axe violation from an inaccessible document', async (testContext) => {
    const browser = await chromium.launch({ channel: 'chromium' });
...
test('reports page and console errors independently', async (testContext) => {
    const browser = await chromium.launch({ channel: 'chromium' });
...
test('navigates to and enforces the strict page policy', async (testContext) => {
    const browser = await chromium.launch({ channel: 'chromium' });
    ...
    try {
        ...
        testContext.after(() => releaseSlowImage(), { timeout: 10000 });
        ...
        const slowPageAssertion = assertStandardPage(page, {
        ...
        const assertNewPageRejects = async (expectation, pattern) => {
            ...
            try {
                await assert.rejects(assertStandardPage(failingPage, expectation), pattern);
            } finally {
        ...
        try {
            ...
            await assert.rejects(
                assertStandardPage(contaminatedPage, {
                    heading: 'Example page',
                    title: 'Example page',
                    url: 'https://example.test/',
                }),
...
test('enforces WCAG AAA and 2.2 rules that stock Axe leaves disabled', async (testContext) => {
    const browser = await chromium.launch({ channel: 'chromium' });
